...
| `responsibility_registry` rows (Delhi) | 4 | All city-wide (`ward_id` null); **updated this pass with real, sourced agency contacts** (MCD `155305`/`mcd-ithelpdesk@mcd.nic.in` for road_dust/construction_dust, Transport Dept `011-23994223`/`comtpt@nic.in` for vehicular, DPCC `011-47441576`/`msdpcc@nic.in` for industrial - each `mapping_source` cites the exact official government page and access date). Still **0 of 4 are `mapping_confidence = 'verified'`** (all `estimated`) - these are real, current, official agency-level contacts, not yet ward-specific human-confirmed routing. `open_burning` deliberately has no row (an intentional "unresolved jurisdiction" test case). `working_hours` and named individual contacts remain unset - not invented. |
...
| `responsibility_registry` has zero verified, ward-specific rows | **Required before pilot** for any ward where operational dispatch will run unattended | **Partially closed this pass**: all 4 rows now carry real, sourced agency-level contacts (see above), so a dispatch at least reaches a real, callable phone number/email today, not a placeholder. Routing will still read `probable` (city-wide match), never `confirmed`, until a real WARD-SPECIFIC verified row exists - `dispatch_intervention_task` still functions, but every dispatch in Delhi today routes to the right AGENCY, not yet a specific ward-scoped unit or named team |
