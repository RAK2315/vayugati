...
echo "   all thirteen re-applied cleanly"

echo "== tests"
fails=0
for t in "$HERE/10_report_to_incident.sql" "$HERE/20_evidence_and_privacy.sql" "$HERE/30_mission_rls.sql" "$HERE/40_intervention_and_impact.sql" "$HERE/50_intervention_playbooks.sql" "$HERE/60_recurrence_and_custom_hardening.sql" "$HERE/70_anomaly_detection.sql" "$HERE/80_source_attribution.sql" "$HERE/90_unified_forecasting.sql" "$HERE/100_authority_routing_and_dispatch.sql" "$HERE/110_production_hardening.sql" "$HERE/120_pilot_validation_scenarios.sql" "$HERE/130_end_to_end_scenarios.sql" "$HERE/140_profile_role_immutability.sql"; do
