...
const STATUS_STYLE: Record<string, string> = {
  ...
  more_evidence_requested: 'bg-status-info/10 text-status-info',
  ...
  return (
    <li className="rounded-xl border border-slate-200 bg-white p-3">
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-sm font-semibold text-slate-800">{RECURRENCE_TYPE_LABEL[report.recurrence_type as keyof typeof RECURRENCE_TYPE_LABEL]}</span>
        <span className={`rounded px-1.5 py-0.5 text-[10px] font-bold uppercase ${STATUS_STYLE[report.review_status] ?? 'bg-slate-100 text-slate-600'}`}>
          {RECURRENCE_REVIEW_STATUS_LABEL[report.review_status as keyof typeof RECURRENCE_REVIEW_STATUS_LABEL] ?? report.review_status}
        </span>
        <span className="ml-auto text-[11px] tabular-nums text-slate-400">{fmtAge(ageHours(report.created_at))} ago</span>
      </div>

      {report.note && <p className="mt-1.5 text-sm text-slate-700">{report.note}</p>}
      <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-[11px] sm:grid-cols-3">
        <div>
          <dt className="text-slate-400">Location given</dt>
          <dd className="font-semibold text-slate-700">{report.lat != null && report.lng != null ? `${report.lat.toFixed(3)}, ${report.lng.toFixed(3)}` : 'Not provided'}</dd>
        </div>
        <div>
          <dt className="text-slate-400">Photo</dt>
          <dd className="font-semibold text-slate-700">{report.photo_url ? 'Attached' : 'None'}</dd>
        </div>
        <div>
          <dt className="text-slate-400">Reported</dt>
          <dd className="font-semibold text-slate-700">{new Date(report.created_at).toLocaleString()}</dd>
        </div>
      </dl>

      <div className="mt-2 rounded-lg bg-accent-50 px-2.5 py-1.5">
        <p className="text-[11px] font-semibold uppercase tracking-wide text-accent-800">
          Recommendation: {decision.recommendation.replace(/_/g, ' ')}
        </p>
        <ul className="mt-1 list-disc pl-4 text-[11px] text-accent-800">
          {decision.reasons.map((r, i) => (
            <li key={i}>{r}</li>
          ))}
        </ul>
        <p className="mt-1 text-[10px] italic text-accent-700">A recommendation only - command makes the final call.</p>
      </div>
      ...
      {isPending && (
        <div className="mt-3 flex flex-wrap gap-1.5">
          <button type="button" disabled={busy} onClick={dismiss} className="focus-ring rounded-lg border border-slate-200 px-2.5 py-1 text-[11px] font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50">
            Dismiss
          </button>
          <button type="button" disabled={busy} onClick={requestEvidence} className="focus-ring rounded-lg border border-slate-200 px-2.5 py-1 text-[11px] font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50">
            Request more evidence
          </button>
          <button type="button" disabled={busy} onClick={confirm} className="focus-ring rounded-lg border border-slate-200 px-2.5 py-1 text-[11px] font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50">
            Confirm recurrence
          </button>
          ...
          <button type="button" disabled={busy} onClick={createLinked} className="focus-ring rounded-lg bg-accent-600 px-2.5 py-1 text-[11px] font-semibold text-white hover:bg-accent-700 disabled:opacity-50">
            Create new linked incident
          </button>
          <button type="button" disabled={busy} onClick={merge} className="focus-ring rounded-lg border border-slate-200 px-2.5 py-1 text-[11px] font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50">
  ...
  return (
    <section className="border-t border-slate-100 px-4 py-3">
      <div className="mb-2 flex items-center gap-2">
        ...
        <span className="rounded bg-slate-100 px-1.5 text-[10px] font-bold text-slate-600">{recurrenceReports.length}</span>
      </div>

      <dl className="mb-3 grid grid-cols-2 gap-x-3 gap-y-1 text-[11px] sm:grid-cols-4">
        <div>
          <dt className="text-slate-400">Closed</dt>
          <dd className="font-semibold text-slate-700">{closedAt ? new Date(closedAt).toLocaleDateString() : 'Unknown'}</dd>
        </div>
        <div>
          <dt className="text-slate-400">Time since closure</dt>
          <dd className="font-semibold text-slate-700">{hoursSinceClosure != null ? fmtAge(hoursSinceClosure) : '-'}</dd>
        </div>
        <div>
          <dt className="text-slate-400">Previous intervention</dt>
          <dd className="font-semibold capitalize text-slate-700">{lastIntervention?.type ?? 'None recorded'}</dd>
        </div>
        <div>
          <dt className="text-slate-400">Previous impact result</dt>
          <dd className="font-semibold capitalize text-slate-700">{lastImpact?.outcome?.replace(/_/g, ' ') ?? 'Not evaluated'}</dd>
        </div>
      </dl>
